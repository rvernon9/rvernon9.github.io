...
function ReverseNameSequence(strName) {
	...
	var intCommaLoc;
	if (strName == "")
		strName = "Not Found, Learner Name";
...
function SCORM2004_GetInteractionCorrectResponses(strInteractionID) {
	...
	if (intCorrectResponseCount == 0)
		return new Array();
	if (intCorrectResponseCount > 1)
		WriteToDebug("SCORM Driver is not currently implemented to support multiple correct response combinations and will only return the first");
...
function SCORM_GrabAPI() {
	...
	//RV switched ordering to look in current window first.
	//   Coursera error: Error grabbing 1.2 API-SecurityError:Blocked a frame with 
	//	 origin "https://d10o6em2qtnr4q.cloudfront.net" from accessing a cross-origin frame.
...
(function () {
	...
	Context.prototype = {
		...
		init: function (cfg) {
			this.log("init");
			var i,
			directProps = ["registration", "revision", "platform", "language", "statement", "extensions"],
...
function ShowDebugWindow(lmsPreview) {
	if (!lmsPreview) {
		...
		if (window.parent) {
			var debugIframe = getDebugLogsIFrame();
			if (debugIframe && debugIframe.contentWindow)
				winDebug = debugIframe.contentWindow;
			else
				console.log("no debugIframe");
		} else {
